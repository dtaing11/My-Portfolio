...
function CodePreview({ sample }: { sample: CodeSample }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        ...
        console.error(err)
        if (!cancelled) setError("Failed to load code snippet.")
      } finally {
  ...
  return (
    ...
    >
      ...
      >
        {loading && <p className="px-2 py-1 text-xs text-slate-200">Loading code...</p>}
            ...
            <Card className="h-full border border-white/10 bg-slate-900/80 backdrop-blur shadow-md shadow-purple-900/40 hover:border-purple-400/50 hover:shadow-purple-500/40 transition-colors">
              <CardHeader>
                <div className="flex items-start justify-between gap-2">
                  ...
                  <span className="text-[11px] font-medium text-amber-300">{project.year}</span>
                </div>
              </CardHeader>
            ...
            <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col">
              ...
              >
                <section className="space-y-4 rounded-xl border border-white/10 bg-slate-900/80 p-4 shadow-xl shadow-purple-900/40 md:p-6">
                  {(activeProject.screens && activeProject.screens.length > 0) || activeProject.image ? (
                  ...
                  <div className="space-y-4">
                    ...
                    {activeProject.highlights?.length ? (
                      <ExpandableList title="Highlights" items={activeProject.highlights} initialCount={3} />
                    ) : null}

                    <ExpandableList title="What I did" items={activeProject.details} initialCount={3} />

                    {activeProject.userExperience?.length ? (
                      <ExpandableList title="User Experience" items={activeProject.userExperience} initialCount={3} />
                    ) : null}

                    <div className="mt-2 flex flex-wrap gap-2 text-[11px] md:hidden">
                      {activeProject.tech.map((t) => (
                        <Badge key={t} variant="outline" className="border-purple-400/60 text-purple-100">
                          {t}
                        </Badge>
                      ))}
                    </div>
                  ...
                  activeProject.codeSamples.length > 0 && (
                    <section className="flex flex-col gap-3 rounded-xl border border-white/10 bg-slate-900/80 p-4 shadow-xl shadow-purple-900/40 md:p-6">
                      ...
                      <div className="min-h-[220px] max-h-[55vh] md:min-h-[340px] md:max-h-none">
                        <CodePreview sample={activeProject.codeSamples[activeSampleIndex]} />
                      </div>
                    </section>
                  )}
              </main>
